...
The use of both test suites, hand made and generated, has proven 
effective on finding protocols errors on the \STSB when the board 
prototypes have been delivered.
...
correct.
The board implementation error has been detected by the 
\lil{StreamDriver} package.

The second prototype delivered was meant to build packets with values
taken from rear sensors installed on the board.  The only sensors that
...
One error have been discovered in the low level driver that the driver
was using; to eliminate the error an upgrade of the low level drover
to a new unreleased version was necessary.  No other errors were
detected on the driver side. All the errors on the \SB side have been
detected.
