...
Agile and formal people do not blend nicely. 
These two communities do not talk to or learn from each other. 
The problem-solving approaches and mind attitudes differ widely, but this clash of view points can be exploited into new development processes that actually blend, rather than fight, or simply compose, practices from the two worlds. 
This paper summarizes a complex case study, showing that it is not only possible, but tasty, to combine the ``garlic'' of formal methods and the ``chocolate'' of agile programming: a ``Mole'' of software verification practices.
