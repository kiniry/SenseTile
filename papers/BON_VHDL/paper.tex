...
\lstdefinelanguage{Bon} {
  morekeywords={require,ensure,feature,old,and,o,Q'slew,end,Result,INTEGER},
  morekeywords={class,delta,invariant,inherit,deferred,effective,NONE,make},
  morecomment=[l]{--}, 
  morecomment=[s]{/*}{*/}, 
  morestring=[b]", 
 literate=,
...
that design and implementation diverge to an inconsistent state. \emph{Seamlessness}
also plays important role in relating behaviours at differing levels of abstraction. 
For example, one may wish to relate the structural views of an A/D converter at the 
level of individual bits and at the level of busses. Therefore capturing similarities 
between the differing levels of abstractions far outweigh their inevitable differences, 
making it possible to obtain a continuous process that ensures a direct mapping between 
a formal specification and its software solution.
...
parameters and theirs constraints. Furthermore any \emph{subtype} represents restricted
set of values from a base \emph{type}. The condition that determines which values are in the
\emph{subtype} may be \emph{contracted} by specifying characteristic predicate. 
For example, the \emph{subtype} \lstinlinenc|bit| of \lstinlinen|INTEGRER| may be defined as:\\
