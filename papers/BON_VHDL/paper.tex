...
\lstdefinelanguage{Bon} {
  ...
  morekeywords={explanation,query,command,constraint,exists},
  morecomment=[l]{--}, 
  morecomment=[s]{/*}{*/}, 
  morestring=[b]", 
 literate=,
...
separatelly in order to produce specification of the \STS. 
Our aim is to be able to precisely describe the structures of circuits,
the behaviour of circuits and the formal link between structures and behaviours.
In other words, the behavioural specification of the \STS may be 
specified as a \inv on the signals at the circuit's ports and may be obtained 
by taking the conjunction of the \emph{invariants} that describe 
behaviours of the individual components separately. 
...
\item 
 ...
 has been used mostly in the pure object oriented software construction.    
\item
...
then it will be derivable from informal BON specification, 
via the refinement calculus to formal BON specification 
and JML implementation.     
\end{enumerate}
As an extremly simple example, 
we will give practical aspect of refinement, applied on real capacitor component, 
...
a "real capacitor" at the level of individual components (a resistor and ideal capacitor)
and their behaviours, or at the more abstract level through general behaviour (\inv) and
using existential quantification (\texttt{exists}) to encapsulate all internal signals. 
Therefore capturing similarities between the differing levels of abstractions 
far outweigh their inevitable differences, making it possible to obtain a 
continuous process that ensures a direct mapping between a formal specification 
and its software solution.
...
We will agree that the constraint of waveform is nothing more than \inv.
Another example of \inv is conservation of energy law in which 
...
(\texttt{terminal}) equals zero. Moreover, any predefined (implicite) attribute in VHDL-AMS  
