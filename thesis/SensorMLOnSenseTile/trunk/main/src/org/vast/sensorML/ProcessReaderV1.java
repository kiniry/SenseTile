...
public class ProcessReaderV1 extends AbstractSMLReader implements ProcessReader
{
    ...
    public ProcessReaderV1()
    {
        ...
        System.out.println("ProcessReaderV1 eeicpr");
        try {
        	//eeicpr
            URL processmap = this.getClass().getResource("/ProcessMap.xml");
            processLoader.loadMaps(processmap.getFile(), true);
    ...
    public DataProcess readProcess(DOMHelper dom, Element processElement) throws SMLException
    {
		...
		if (dom.existElement(processElement, "Component/inputs"))
			System.out.println("readprocess exist");
        ...
        if (dom.existElement(processElement, "components")){
            ...
            System.out.println("components "+processElement.getLocalName());
        }
        else
    ...
    public DataProcess readProcessModel(DOMHelper dom, Element processModelElement) throws SMLException
    {
        ...
        System.out.println(processModelElement.getLocalName()+" urn "+uri);
        // load process
        DataProcess newProcess;
        //eeicpr
        System.out.println("createExecutableProcess "+createExecutableProcess);
        ...
        try
        {
            if (portType.equals("inputs"))
            {
                if (internalConnection)
                {
                    ...
                    System.out.println("internalConnectionInput "+portName);
                }
                else
                {
                    selectedProcess.connectInput(portName, dataQueue);
                    System.out.println("ConnectionInput "+portName);
                }
            }
            else if (portType.equals("outputs"))
            {
                if (internalConnection)
                {
                    ...
                    System.out.println("InternalConnectionOutput "+portName);

                }
                else
                {
                    selectedProcess.connectOutput(portName, dataQueue);
                    System.out.println("ConnectionOutput "+portName);
    ...
    public void readProcessIO(DOMHelper dom, Element processElement, DataProcess process) throws SMLException
    {
        try
		{
			...
			NodeList signalList;
			int signalCount;
			System.out.println("readProcesIO"+processElement.getLocalName());
			// read inputs
			...
			for (int i=0; i<signalCount; i++)
			{
			    ...
			    System.out.println("Input data - "+inputData.getName());
			}
