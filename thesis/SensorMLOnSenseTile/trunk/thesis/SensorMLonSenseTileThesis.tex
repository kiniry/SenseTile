...
\end{figure}
The inheritance heirarhcy is changed slightly with System inheriting from ProcessChain and Component inheriting from ProcessModel. A new abstraction called Port is added. Port contains all the properties of SensorML inputs, outputs and parameters with classes Input, Output and Parameter subtyping to make mapping to SensorML easier.
...
\section{BON to SensorML XML Mapping}
...
The MetaDataGroup is not described in BON as explained previously and only a place holder value is mapped from this feature.

\section{ProcessChain connections problem}
One of the major features of SensorML is the ablity to describe chains of processes. The SensorML ProcessChain and System types have a connections property that allows this chaining as decribed in section \ref{SMLref} .However strings are used to describe the connection. These connection strings are parsed to find out the inputs and outputs to connect up as described in SenseTile System section \ref. This is very error prone and the ability to check that connections are correct to a least some degree is desirable.
Below are two fragents form SenseTile System and SenseTile Thermistor that descibe a connection form th 
...
\begin{lstlisting}
 ...
 BON specification can enforce some level of correctness in that inputs in connections must be inputs in the components contained in that chain. This is shown in the BON fragment below.
